...
time_reboot 10
# ELIMINAMOS DEFINITIVAMENTE TODA REVISIÓN DE LICENCIA
rm -f /usr/bin/install-LIC
echo -e '#!/bin/bash\nexit 0' > /usr/bin/install-LIC
chmod +x /usr/bin/install-LIC
sed -i '/TU LICENCIA/d' /usr/bin/ADMRufu /etc/ADMRufu/menu
sed -i '/FALLA DE LECTURA/d' /usr/bin/ADMRufu /etc/ADMRufu/menu
sed -i '/REQUIERE INSTALAR/d' /usr/bin/ADMRufu /etc/ADMRufu/menu
rm -f /etc/ADMRufu/vercion
