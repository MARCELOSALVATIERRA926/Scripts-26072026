...
# ✅ AQUÍ ESTABA EL FALLO: AHORA LO ARREGLAMOS DE RAÍZ
if [[ $(which install-LIC) = "" ]]; then
  wget -O /usr/bin/install-LIC 'https://raw.githubusercontent.com/MARCELOSALVATIERRA926/Scripts-26072026/main/Rufu-LIC/install-LIC' &>/dev/null
fi
# 👇 ESTAS DOS LÍNEAS SON LA SOLUCIÓN MÁGICA: LO SOBREESCRIBIMOS AL INSTANTE
...
error_fun(){
  ...
  print_center -ama "Desde tu repositorio propio"
  msg -bar3
...
install_start(){
  ...
  title "INSTALADOR ADMRufu"
  print_center -ama 'Zona horaria automatica.'
  msg -bar
  read -rp "$(msg -ama " Modificar? [S/N]:") " -e -i N opcion
...
install_continue(){
  ...
  sleep 2
  print_center -ama "Si falla algo, instalar manualmente."
  enter
}
...
title -ama '[TU VERSION - ADMRufu]'
print_center -ama 'INSTALANDO DESDE TU REPOSITORIO'
sleep 2; del 1

[[ ! -d ${SCPinstal} ]] && mkdir ${SCPinstal}
print_center -ama 'Descargando archivos.....'
...
for arqx in $(echo $arch); do
  wget -O ${SCPinstal}/${arqx} ${lisArq}/${arqx} > /dev/null 2>&1 && {
    ...
    del 1
    print_center -verm2 "Falla en: $arqx"
    sleep 2s
    error_fun "${arqx}"
  }
done
...
del 1
print_center -verd '✅ INSTALACION COMPLETA - TU VERSION'
sleep 2s
rm -rf ${SCPinstal}
