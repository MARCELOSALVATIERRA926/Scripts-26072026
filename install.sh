...
# ✅ TU VALIDACIÓN FÉNIX - ACEPTA CON O SIN TILDE
if [[ $(which install-LIC) = "" ]]; then
  cat > /usr/bin/install-LIC << 'EOF'
#!/bin/bash
read -p "🔑 INGRESAR CLAVE DE LICENCIA: " CLAVE
CLAVE_LIMPIA=$(echo "$CLAVE" | tr '[:lower:]' '[:upper:]' | sed 's/É/E/g')
if [[ "$CLAVE_LIMPIA" == "FENIX" ]]; then
  echo "✅ LICENCIA CORRECTA - ACCESO PERMITIDO"
  exit 0
else
  echo "❌ NO ES SU CLAVE - INSTALACIÓN DETENIDA"
  exit 1
fi
EOF
...
install_start(){
  title "INSTALADOR ADMRufu"
  print_center -ama "A continuacion se actualizaran los paquetes del systema."
  msg -bar3
...
install_continue(){
  ...
  sleep 2
  print_center -ama "si algunas falla, instalar manualmente."
  enter
}
...
# ==================================================
# ✅ LIMPIEZA AUTOMÁTICA DE VALIDACIÓN ORIGINAL
# ==================================================
...
print_center -ama '🔑 VALIDACIÓN CONFIGURADA: FÉNIX'
print_center -ama '✅ TODOS LOS CONTROLES ORIGINALES ELIMINADOS'
sleep 3
...
clear
title "-- TU VERSION ADMRufU | FÉNIX --"
