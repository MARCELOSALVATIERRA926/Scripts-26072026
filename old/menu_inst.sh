# Borramos el viejo
rm -f /etc/ADMRufu/menu /usr/bin/ADMRufu /usr/bin/menu /usr/bin/adm

# Creamos el menú nuevo (pegá acá todo el código que me mandaste, y al principio agregamos esto):
cat > /etc/ADMRufu/menu << 'EOF'
# ✅ AGREGADO: CARGAMOS FUNCIONES ANTES DE EMPEZAR
source /etc/ADMRufu/module

# ==== AQUÍ PEGAS TODO EL CÓDIGO QUE ME MANDASTE TAL CUAL ====
cache_ram() {
...
new_banner(){
  ...
  if [[ "${opcion}" ]]; then
    ...
    echo "$opcion" > $local
    [[ ! -e ${ADM_tmp}/message.txt ]] && echo "@FENIX-M&M" > ${ADM_tmp}/message.txt
    credi="$(less ${ADM_tmp}/message.txt)"
    echo '<h4 style=text-align:center><font color="#047980">F</font><font color="#0d6e74">E</font><font color="#006462">N</font><font color="#185260">I</font><font color="#006462">X</font><font color="#0d6e74">-</font><font color="#047980">M&M</font><br><font color="#047980">'$credi'</font></h4>' >> $local
...
pid_inst(){
  ...
  for list in $proto; do
    case $list in
      ...
      python) portas2=$(echo "$portas"|grep -w "LISTEN"|grep -w "$list") && [[ $(echo "${portas2}"|grep "$list") ]] && inst[$list]="\033[1;32m[ON] " || inst[$list]="\033[1;31m[OFF]";;
      ttdns|openvpn) portas2=$(echo "$portas"|grep -w "LISTEN"|grep -w "$list") && [[ $(echo "${portas2}"|grep "$list") ]] && inst[$list]="\033[1;32m[ON] " || inst[$list]="\033[1;31m[OFF]";;
    esac
  done
...
# Damos permisos y enlazamos
chmod +x /etc/ADMRufu/menu
ln -sf /etc/ADMRufu/menu /usr/bin/ADMRufu
ln -sf /etc/ADMRufu/menu /usr/bin/menu
ln -sf /etc/ADMRufu/menu /usr/bin/adm

# Creamos tu validación para que no pida nada más
mkdir -p /etc/ADMRufu
echo "OK" > /etc/ADMRufu/acceso.valido
chmod 600 /etc/ADMRufu/acceso.valido
